...
class ReportScheduler:
    ...
    async def send_sector_report(self, chat_id: int, sector_id: Optional[int] = None) -> bool:
        """
        Отправить отчет по сектору в указанный чат
        
        Args:
            chat_id: ID чата для отправки
            sector_id: ID сектора (опционально)
        ...
        try:
            ...
            if len(formatted_report) > 4000:
                ...
                await self.bot.send_message(chat_id, formatted_report, parse_mode="Markdown")
    ...
    async def send_all_sectors_reports(self):
        """Разослать отчеты по всем секторам"""
        ...
        try:
            ...
            for sector in sectors:
                ...
                # Получаем админов этого сектора
                #admins = await self.get_sector_admins(sector_id)
                
                #if not admins:
                #    logger.warning(f"  ⚠️ Нет админов для сектора {sector_id}")
                #    continue
        ...
        try:
            ...
            if isinstance(users_data, dict):
                ...
                if "error" in users_data:
                    logger.error(f"Ошибка API при получении пользователей: {users_data['error']}")
                    return []
            ...
            for user in users:
                try:
                    ...
                    if sector_id is None or user_sector == sector_id:
                        ...
                        logger.debug(f"  Найден админ: {user_id} (сектор: {user_sector})")
